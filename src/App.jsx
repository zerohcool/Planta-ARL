...
function App() {
  ...
  const porcPetroleoFabricado = mezclaFabricada
    ? (Number(petroleoUsar) / mezclaFabricada) * 100
    : 0;

  const porcAceiteFabricado = mezclaFabricada
    ? (Number(aceiteUsar) / mezclaFabricada) * 100
    : 0;
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-xl shadow mt-8">
          <h2 className="text-lg font-semibold mb-4">Informe de movimientos</h2>
            ...
            <tbody>
              {movimientos.map((mov, i) => (
                <tr key={i} className="border-b">
                  <td className="py-2">{mov.producto}</td>

                  <td
                    className={`py-2 font-semibold ${mov.cantidad > 0 ? "text-green-600" : "text-red-600"}`}
                  >
                    {mov.cantidad > 0 ? "+" : ""}
                    {mov.cantidad} m³
                  </td>
